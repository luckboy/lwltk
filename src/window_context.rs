...
impl WindowContext
{
    ...
    ///
    /// The current window index refers the window for a wayland event that is called or a thread
    /// signal.
    ...
    ///
    /// The current position is pointed by the pointer or the touch for a wayland event that is
